	...
	class Itallap extends Controller {		
		function main() {			
			...
			$this->view->all_products = $this->db->Query("SELECT products.*, categories.title cat_title FROM products 
				...
				WHERE categories.parent_id='134' ORDER BY categories.sort_order, products.sort");
			$productsByCat = array();
			...
			$qb = QueryBuilder::QueryBuilder()->Select()->From("categories")->Where("parent_id='134'")->OrderBy("sort_order");			
			...
			$category_id = $this->db->Query("SELECT * FROM categories WHERE parent_id='134' ORDER BY sort_order LIMIT 0,1")->id;			
			}						
