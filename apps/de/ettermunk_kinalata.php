	...
	class Ettermunk_kinalata extends Controller {
		...
		function main() {
			...
			$this->view->all_products = $this->db->Query("SELECT products.*, categories.title cat_title FROM products 
											  ...
											  WHERE categories.parent_id='105' AND categories.id!='143' ORDER BY categories.sort_order, products.sort");
			$productsByCat = array();
			...
			$qb = QueryBuilder::QueryBuilder()->Select()->From("categories")->Where("id!='143'")->Where("parent_id='105'")->OrderBy("sort_order");
			...
			$this->view->display("de/ettermunk_kinalata");
		}
	}
?>
