...
class Fejleckepek_edit extends Controller {
	...
	private function RejectInvalidCrop($redirect_url, $crop_data) {
		...
		error_log("Rejected invalid cover crop in fejleckepek/edit: '" . json_encode($crop_data) . "'");
	...
	private function RejectInvalidPicture($redirect_url, $picture_filename) {
		...
		error_log("Rejected invalid cover image in fejleckepek/edit: '" . $picture_filename . "'");
		URI::Redirect($redirect_url);
		exit;
	}
	...
	function Submit($data) {
		$redirect_url = ModuleHelper::GetFunctionLink("admin/edit", array("id" => URI::GetNamedParam("id")));
